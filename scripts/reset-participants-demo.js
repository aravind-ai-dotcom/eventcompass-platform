// =============================================================================
// EventCompass — Participant Demo Reset
// scripts/reset-participants-demo.js
//
...
//
// Usage:
//   node scripts/reset-participants-demo.js
//
// Prerequisites:
...
// ── Consent preset factory ─────────────────────────────────────────────────────
// Six preset variations as required
...
async function run() {
  ...
  for (const d of snap.docs) {
    await deleteDoc(doc(db, `${BASE}/participants/${d.id}`));
    deleted++;
  }
  ...
  for (const p of PARTICIPANTS) {
    await setDoc(doc(db, `${BASE}/participants/${p.id}`), p);
