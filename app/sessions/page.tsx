...
// =============================================================================
// EventCompass — Sessions Catalog  /sessions
//
// An intelligent session catalog — not a static list.
// Sessions are scored against participant ATT-0001 using the same
// 9-signal weighted model as /experience.
//
// Sections:
//   1. Hero         — title, subtitle, live counts
//   2. Filters      — search, track, type, day (client-side, instant)
//   3. Recommended  — top 6 scored sessions with reasons
//   4. Full catalog — all sessions in score order with compact rows
//
// Firestore:
//   organizations/ibm/events/txc2026/participants/ATT-0001
//   organizations/ibm/events/txc2026/sessions
//
// Rules:
//   No Firestore writes. No auth. No pagination yet.
//   globals.css class names only — no new CSS.
// =============================================================================
...
const W = {
  track: 25, goal: 20, need: 15, role: 10,
  industry: 10, keyword: 5, executive: 5, broad: 5, handsOn: 5,
} as const;
...
// ─────────────────────────────────────────────────────────────────────────────
// Utilities  (same pattern as experience page)
// ─────────────────────────────────────────────────────────────────────────────

function lower(items: (string | undefined | null)[]): string[] {
  return items.filter((v): v is string => typeof v === "string" && v.trim() !== "").map(v => v.toLowerCase());
}
...
// ─────────────────────────────────────────────────────────────────────────────
// Scoring  (exact same weights as experience/page.tsx)
// ─────────────────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────────
// Sub-components (inline — no separate files yet)
// ─────────────────────────────────────────────────────────────────────────────
...
function ScoreBadge({ score }: { score: number }) {
  ...
  return (
    <div
      ...
      style={{ minWidth: "46px", minHeight: "46px", flexShrink: 0 }}
...
export default function SessionsPage() {
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        const pData = pSnap.exists() ? (pSnap.data() as RawDoc) : ({} as RawDoc);
  ...
  const { tracks, types, days } = useMemo(() => {
    ...
    for (const s of allScored) {
      const t = primaryTrack(s); if (t) trackSet.add(t);
      typeSet.add(sessionType(s));
      const d = sessionDay(s); if (d) daySet.add(d);
    }
    return {
    ...
    return (
      <section className="section no-top-border">
        ...
        <p style={{ color: "var(--muted)" }}>
          Reading from{" "}
          <span style={{ fontFamily: "var(--font-mono, ui-monospace)", color: "var(--accent)" }}>
            {BASE}/sessions
          </span>
        </p>
      </section>
  ...
  if (status === "error") {
    return (
      <section className="section no-top-border">
        ...
        <h2 style={{ fontSize: "clamp(1.8rem,3vw,3rem)", fontWeight: 520, letterSpacing: "-0.04em", margin: "12px 0 16px" }}>
    ...
    <>
      ...
      <section className="section no-top-border">
        ...
        {tracks.length > 2 && (
          <div style={{ marginBottom: "14px" }}>
            <p style={{ color: "var(--muted)", fontSize: "0.72rem", textTransform: "uppercase", letterSpacing: "0.1em", fontWeight: 680, margin: "0 0 8px" }}>
              Tech track
            </p>
        ...
        {isFiltered && (
          <div style={{ display: "flex", alignItems: "center", gap: "16px", marginTop: "8px" }}>
            <span style={{ color: "var(--muted)", fontSize: "0.88rem" }}>
              {filtered.length} of {totalCount} sessions
            </span>
            <button
          ...
          <div className="section-head">
            <div>
              <div className="section-kicker">
                {isFiltered ? "Top matches in results" : "Recommended for you"}
              </div>
              <h2>
                {isFiltered
                  ? `Top ${recommended.length} from your search`
                  : "Your highest-scored sessions."}
              </h2>
            </div>
            <p>
              {isFiltered
                ? "Compass scores within your filtered results. Score reflects match to your profile."
                : "Ranked by track alignment, goals, needs, role, industry, and keyword overlap. Reasons show why each session matched."}
            </p>
          </div>
      ...
      <section className="section">
        <div className="section-head">
          <div>
            <div className="section-kicker">
              {isFiltered ? "Filtered catalog" : "All sessions"}
            </div>
            <h2>
              {isFiltered
          ...
          <p>
            Sorted by Compass score — highest match first.
            Sessions with no score match are shown at the bottom.
          </p>
        </div>
