...
function Panel({ children }: { children: React.ReactNode }) {
  return (
    <div style={{ background: "rgba(255,255,255,0.03)", border: `1px solid ${ADMIN_SURFACE.border}`, padding: "18px 20px" }}>
      {children}
    </div>
  );
}
...
function CountRow({ label, count, total }: { label: string; count: number; total: number }) {
  return (
    <div style={{ display: "flex", justifyContent: "space-between", gap: "12px", padding: "6px 0", borderBottom: `1px solid ${ADMIN_SURFACE.border}` }}>
      <span style={{ color: ADMIN_SURFACE.muted, fontSize: "0.84rem" }}>{label}</span>
      <span style={{ color: ADMIN_SURFACE.text, fontSize: "0.84rem", fontVariantNumeric: "tabular-nums" }}>
        {count.toLocaleString()} <span style={{ color: ADMIN_SURFACE.dim }}>({pctOf(count, total)}%)</span>
      </span>
    </div>
  );
}
...
function Card({
  ...
  return (
    <Panel>
      <p style={{ color: ADMIN_SURFACE.accent, fontSize: "0.68rem", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.1em", margin: "0 0 12px" }}>
        {title}
      </p>
      {rows.map(row => (
  ...
  return (
    <div>
      <div style={{ marginBottom: "24px" }}>
        <p style={{ color: ADMIN_SURFACE.accent, fontSize: "0.68rem", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.1em", margin: "0 0 8px" }}>
          Attendees
        </p>
        <h1 style={{ color: ADMIN_SURFACE.text, fontSize: "1.75rem", fontWeight: 520, margin: "0 0 8px", letterSpacing: "-0.02em" }}>
          Identity Signals
        </h1>
        <p style={{ color: ADMIN_SURFACE.muted, fontSize: "0.9rem", margin: 0, maxWidth: "52rem", lineHeight: 1.5 }}>
