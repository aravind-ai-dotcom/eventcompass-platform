...
export default function SkoEnrollView() {
  const {
    user,
    profile,
    profileComplete,
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        setDataError("Could not load enrollment options from Firestore.");
      } finally {
  ...
  if ((loading || dataLoading) && !profileTimedOut && !dataTimedOut) {
  ...
  return (
    <section className="sko-section sko-enroll">
      <header className="sko-enroll-header">
        <p className="sko-kicker">Intent enrollment · SKO2H 2026</p>
        <h1>Build Your SKO Compass</h1>
        <p className="sko-lead">
          Tell Compass about your persona, geo, market, and goals — we will shape your seller briefing.
        </p>
      </header>

      {(dataError || dataTimedOut) && (
        <p className="sko-muted sko-help-note">
          {dataError ?? "Enrollment options took longer than expected."} Using local SKO defaults where needed.
        </p>
      )}
      ...
      <div className="sko-enroll-section">
        ...
        <div className="sko-field-row">
          <label className="sko-field">
            <span>Geo</span>
            <select className="sko-select" value={geoId} onChange={e => setGeoId(e.target.value)}>
              {geos.map(geo => (
